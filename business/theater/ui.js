import { ensureTheaterHtml } from './html.js';

// 棱 UI 只编排注入的宿主能力；它不读取 SillyTavern 全局对象，也不拥有生成状态。
export function createTheaterUi({ theater, feature, host = {} } = {}) {
    const esc = host.escapeHtml || (value => String(value ?? '').replace(/[&<>"']/g, ch => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[ch])));
    const attr = host.escapeAttr || esc;
    const state = { current: null, templates: [], source: null, lastRandom: null, bound: false, fsEsc: null, settingsRoots: [], generationSeq: 0, abortPending: false };
    ...
    const findPiece = id => [...(theater.loadDrafts(currentChat()) || []), ...(theater.loadSaved() || [])].find(piece => String(piece.id) === String(id));
    ...
    const render = () => {
        const drafts = (theater.loadDrafts(currentChat()) || []).slice().reverse(); const saved = (theater.loadSaved() || []).slice().reverse();
        ...
        const safeHtml = piece ? ensureTheaterHtml(piece.html, piece.raw) : '';
    ...
    const bind = root => {
        ...
        root.on('click.sp-theater-ui', '.sp-theater-del-draft', async function () { const target = theater.captureTheaterTarget(currentChat()); const id = host.data?.(this, 'id'); if (await host.confirm?.('删除草稿', '确定删除这条小剧场草稿吗？') && isCurrent(target)) { theater.deleteDraft(target.chatId, id); render(); } });
        root.on('click.sp-theater-ui', '.sp-theater-promote', function () { const target = theater.captureTheaterTarget(currentChat()); const piece = (theater.loadDrafts(target.chatId) || []).find(p => p.id === host.data?.(this, 'id')); if (!piece) return; Promise.resolve(theater.promoteToSaved(target.chatId, piece, target)).then(result => { if (result?.ok && isCurrent(target)) { host.toast?.('已永久保存'); render(); } else if (!result?.cancelled && isCurrent(target)) host.toast?.('永久保存失败，请重试', null, true); }).catch(() => { if (isCurrent(target)) host.toast?.('永久保存失败，请重试', null, true); }); });
        root.on('click.sp-theater-ui', '.sp-theater-del-saved', async function () { const target = theater.captureTheaterTarget(currentChat()); if (!await host.confirm?.('删除永久保存', '确定从本对话删除这条已永久保存的小剧场吗？删除后无法恢复。') || !isCurrent(target)) return; try { const result = await theater.deleteSaved(target.chatId, host.data?.(this, 'id'), target); if (isCurrent(target)) { if (!result?.ok && !result?.cancelled) host.toast?.('删除永久稿失败，请重试', null, true); render(); } } catch { if (isCurrent(target)) host.toast?.('删除永久稿失败，请重试', null, true); } });
        root.on('click.sp-theater-ui', '.sp-theater-save', async function () { if (!state.current) return; const target = theater.captureTheaterTarget(currentChat()); const title = String(host.val?.('#sp-theater-title') || '').trim(); state.current.title = title; const draftResult = theater.updateDraft?.(target.chatId, state.current.id, { title }); if (draftResult?.ok === false) { if (isCurrent(target)) host.toast?.('永久保存失败，请重试', null, true); return; } try { const result = await theater.promoteToSaved(target.chatId, state.current, target); if (result?.ok && isCurrent(target)) { host.toast?.('已永久保存到本对话'); render(); } else if (result?.ok === false && isCurrent(target)) host.toast?.('永久保存失败，请重试', null, true); } catch { if (isCurrent(target)) host.toast?.('永久保存失败，请重试', null, true); } });
        root.on('click.sp-theater-ui', '.sp-theater-regen', function () { if (feature.busy || !state.current) return; const regen = theater.resolveTheaterRegen(state.current, host.val?.('#sp-theater-input') || ''); if (!regen.input) { host.toast?.('旧草稿未记录原主题，请先填写输入', null, true); host.focus?.('#sp-theater-input'); return; } state.source = regen.templateSource; host.val?.('#sp-theater-input', regen.input); generate(regen.input); });
    ...
    const measureFold = () => {
        ...
        host.onPendingResultImagesLoad?.(measure);
    };
    ...
    const resetForChat = () => { closeVisual(); state.generationSeq++; state.abortPending = false; state.current = null; state.source = null; state.templates = []; };
    const cleanup = root => {
        root?.off?.('.sp-theater-ui');
        for (const entry of state.settingsRoots) entry.root?.off?.('.sp-theater-ui');
        state.settingsRoots = [];
        state.bound = false;
        resetForChat();
    };
    return { render, bind, bindSettings, refreshTemplates, generate, closeVisual, resetForChat, cleanup, get state() { return state; } };
}
