...
function trackedLinesForPrompt(previousRaw) {
    ...
    if (!tracked.length) return previousRaw;
    // pin 是本地面板的内部状态，不把真实用户锁暴露给模型，避免模型照抄。
    return serializeLines(tracked.map(line => ({ ...line, pin: false })));
}
...
【当前已追踪】\n${trackedLinesForPrompt(previousRaw)}`.replace(/\{\{user\}\}/g, userName).replace(/\{\{char\}\}/g, charName);
}
