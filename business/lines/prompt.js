...
function vectorPromptContext(vectorContext = {}) {
    ...
    const poolContract = hasPools ? '\n本轮新线输出顺序自由，不要求先 SFW 后 NSFW；但必须使用两张 SFW 票，并且 NSFW 票只能使用从第一张开始的连续前缀。目标为 2 条 SFW + 5 条 NSFW；质量不足时可输出 2+4、2+3 等，不得漏掉 SFW、跳过 NSFW 票或在 NSFW 前缀后再使用后面的票。SFW 新线不得成人化，NSFW 新线必须以成人欲望、成人场景或成人互动本身为核心，不得把普通权谋、任务或交易线只在 Next 尾部强行性化。同一主角或同一组参与者可以重复；成人线至少在场景、关系结构、互动机制、节奏或即时身体/关系后果之一有实质差异，不得为换人物凭空发明无关角色。' : '';
