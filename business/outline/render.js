...
export function createOutlineRenderer({ escapeHtml, cleanText, makeInjectButton, makeCopyButton } = {}) {
    ...
    const empty = () => `<div class="sp-empty"><i class="fa-solid fa-scroll"></i><p>当前还没有面，可以先直接聊天讨论，也可以生成一版面作为起点</p><button class="sp-gen-btn sp-outline-gen-btn" id="sp-gen-outline-now">生成面</button></div>`;
    const render = (raw, cursor = 0) => {
        ...
        const cards = beats.map((beat, index) => {
            ...
            const injectButton = makeInjectButton?.(injectParts.join('\n')) || '';
