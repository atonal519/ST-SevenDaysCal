...
export function renderAxisCalendar(env) {
    ...
    for (let day = 1; day <= dim; day++) {
        ...
        const dots = dayItems.length
            ...
            : '';
        cells.push(`<div class="sp-alm-cell${dayItems.length ? ' sp-alm-cell-has' : ''}${anchor.month === month1 && day === anchor.day ? ' sp-alm-cell-today' : ''}${selected === day ? ' sp-alm-cell-sel' : ''}" data-day="${day}"><span class="sp-alm-cell-num">${day}</span>${dots}</div>`);
    }
    let detailItems;
    let detailHead;
    if (selected != null) {
